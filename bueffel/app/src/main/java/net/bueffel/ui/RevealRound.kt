...
fun RevealRound(
    ...
    Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
        ...
        if (!shown) {
            BueffelButton(text = "Lösung zeigen", onClick = { shown = true })
        } else {
            Caption(text = "LÖSUNG")
            ...
            BueffelButton(text = "Hatte ich", onClick = { onSubmit(true) })
            Spacer(Modifier.height(10.dp))
            BueffelButton(text = "Hatte ich nicht", onClick = { onSubmit(false) }, filled = false)
        }
